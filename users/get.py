...
##
# Get an individual user option from the dynamodb Table
##
def user(event, context):
    logger.info("Entering get user")
    ...
    if ('pathParameters' not in event) or ('id' not in event['pathParameters']):
        logger.error("No user id supplied in event.")
        raise Exception("No user id supplied in event.")
    ...
    if not result.get("Item"):
        logger.error("No user option to Get")
        response = {
            "statusCode": 400,
